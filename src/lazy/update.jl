...
function LazyUpdateState(gen_fn, args, prev_trace, constraints, params)
    visitor = AddressVisitor()
    discard = choicemap()
    trace = LazyTrace(gen_fn, args)
    LazyUpdateState(prev_trace, trace, constraints,
...
function traceat(state::LazyUpdateState, gen_fn::GenerativeFunction{T,U},
...
function splice(state::LazyUpdateState, gen_fn::DynamicDSLFunction,
                args::Tuple)
...
function update_delete_recurse(prev_trie::PTrie{Any,ChoiceOrCallRecord},
...
function add_unvisited_to_discard!(discard::LazyChoiceMap,
                                   visited::DynamicSelection,
